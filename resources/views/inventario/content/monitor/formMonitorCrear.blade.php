<style>
    #modal_registrar_monitor .form-group select,
    #modal_registrar_monitor .form-group option,
    #modal_registrar_monitor .form-group input,
    #modal_registrar_monitor .form-group textarea,
    #modal_registrar_monitor .form-group label
    {
        /* font-size: 14px; */
        color: #3a3b45;
    }
...
<form action="{{asset('/inventario/monitor_create')}}" id="form_registrar_monitor" route="{{ route('inventario.monitor_create') }}" novalidate>
    @csrf
    <div class="row">
            ...
            <div class="form-group mb-1">
                ...
                <div class="input-group">
                    ...
                    <div class="input-group-append">
                        <button id="btn_comprobar_serie_monitor" class="btn btn-info btn-sm" type="button" id="button-addon2"><i class="fas fa-check-circle"></i></button>
                        <button id="btn_cambiar_serie_monitor" class="btn btn-warning btn-sm" type="button" id="button-addon2"><i class="fas fa-pen fa-sm"></i></button>
                    </div>
                    
                    <div class="serie-feedback invalid-feedback">
                        Ingrese número de serie (min. 6 caracteres)
                    </div>
                    <input type="hidden" id="route_comprobar_serie_monitor" value="{{ route('inventario.comprobar_serie_monitor') }}">
                </div>
            </div>
        </div>
        <div class="col-lg-4 col-md-12 col-sm-12">
        ...
        <div class="col-lg-4 col-md-12 col-sm-12">
            <div class="form-group">
                ...
                <select class="form-control" id="modelo_id" name="modelo_id">
                    <option value=""></option>
                    @foreach ($modelo_monitor as $item)
       ...
       {{--  <div class="col-lg-4 col-md-12 col-sm-12">
            <div class="form-group">
                ...
                <select class="form-control" id="categoria_id" name="categoria_id">
                    <option value=""></option>
                    @foreach ($categoria_monitor as $item)
                        <option value="{{$item->id}}">{{$item->nombre}}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 text-center">
            <button type="button" id="btn_guardar_form_monitor" route="{{ route('inventario.monitor_create') }}" class="btn btn-success btn-md">Guardar</button>
        </div>
    </div>
</form>

<script>
    var cerrar_modal_reg_monitor = false;
    $(document).ready(function () {

        //bloquea campos iniciales form registro PC
        bloqueaCamposFormRegistrarMonitor();

        $('#modal_registrar_monitor .close').on('click', function (event) {
           console.log(event);
           event.preventDefault(); // Previene el cierre del modal
           event.stopPropagation(); // Detiene la propagación del evento

           if (!cerrar_modal_reg_monitor){
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        cerrar_modal_reg_monitor = true;
                        $('#modal_registrar_monitor').modal('hide');
                        //$('#body_modal_registrar_monitor').html('');
                        console.log('confirma salir');
                        
                    } else {
                        cerrar_modal_reg_monitor = false;
                    }
                });
           }
        ...
        $('#modal_registrar_monitor').on('hidden.bs.modal', function () {
        ...
        $('#numero_sala').select2({
            ...
            dropdownParent: $("#form_registrar_monitor")
        ...
        $('#btn_cambiar_serie_monitor').on('click', function (event) {
            event.preventDefault();
            ...
            bloqueaCamposFormRegistrarMonitor();
            $('#btn_comprobar_serie_monitor').prop('disabled',false);
            setTimeout(() => {
                $('#serie').focus();
            }, 150);
        });

        $('#btn_comprobar_serie_monitor').on('click',async function (event) {
           event.preventDefault();
                ...
                if (val_serie && val_serie.length > 0 && val_serie.length < 6 ){
                    ...
                    await comprobar_serie_monitor(val_serie);
       ...
       /*  $('#ip_id').select2({
        ...
        $('#btn_guardar_form_monitor').on('click', function (event) {
            event.preventDefault();
            ...
            if (!document.getElementById('sala_id').value){
                ...
                if (document.getElementById('edificio_piso').value){
                    ...
                    if (!document.getElementById('numero_sala').value){
                        ...
                        return_ = false;
                        $('#numero_sala').prop('disabled',false).removeClass('is-valid is-invalid').addClass('is-invalid').focus();
                        Swal.fire({
    ...
    function bloqueaCamposFormRegistrarMonitor(){

        $('#form_registrar_monitor input').prop('disabled',true);
        $('#form_registrar_monitor select').prop('disabled',true);
        $('#form_registrar_monitor textarea').prop('disabled',true);
        $('#form_registrar_monitor button').prop('disabled',true);
        $('#form_registrar_monitor #serie').prop('disabled',false);
        $('#btn_comprobar_serie_monitor').prop('disabled',false);
        $('#btn_cambiar_serie_monitor').prop('disabled',false);
        
    }

    function ActivaCamposFormRegistrarMonitor(){

        //await new Promise(resolve => setTimeout(resolve, 500));
        //await new Promise(resolve => (resolve));

        $('#form_registrar_monitor input').prop('disabled',false);
        $('#form_registrar_monitor select').prop('disabled',false);
        $('#form_registrar_monitor textarea').prop('disabled',false);
        $('#form_registrar_monitor button').prop('disabled',false);
    ...
    async function comprobar_serie_monitor(serie){

        console.log(serie);
        loader('show');
        try {
            
            const route = document.getElementById('route_comprobar_serie_monitor').value;
            ...
            if (data.status == 200){
                ...
                if (data.serie){
                    ...
                    ActivaCamposFormRegistrarMonitor();
                    //bloqueaCamposAppsFormRegistrarPc();
                    $('#btn_comprobar_serie_monitor').prop('disabled',true);
