<style>
    #modal_editar_monitor .form-group select,
    #modal_editar_monitor .form-group option,
    #modal_editar_monitor .form-group input,
    #modal_editar_monitor .form-group textarea,
    #modal_editar_monitor .form-group label
    {
        /* font-size: 14px; */
        color: #3a3b45;
    }
...
<form action="{{asset('/inventario/monitor_editar')}}" id="form_editar_monitor" route="{{ route('inventario.monitor_update') }}" novalidate>
    @csrf
    <div class="row">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 text-center">
            <button type="button" id="btn_editar_form_monitor" route="{{ route('inventario.monitor_update') }}" class="btn btn-warning btn-md">Editar</button>
            <input type="hidden" id="monitor_id_editar" name="monitor_id_editar" value="{{$monitor_id}}">
        </div>
    </div>
</form>

<script>
    var cerrar_modal_edit_monitor = false;
    $(document).ready(function () {

        //bloquea campos iniciales form registro PC
        //bloqueaCamposFormRegistrarPC();

        $('#modal_editar_monitor .close').on('click', function (event) {
           console.log(event);
           event.preventDefault(); // Previene el cierre del modal
           event.stopPropagation(); // Detiene la propagación del evento

           if (!cerrar_modal_edit_monitor){
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        cerrar_modal_edit_monitor = true;
                        $('#modal_editar_monitor').modal('hide');
                        console.log('confirma salir');
                        
                    } else {
                        cerrar_modal_edit_monitor = false;
                    }
                });
           }
        ...
        $('#modal_editar_monitor').on('hidden.bs.modal', function (event) {
            console.log('El modal editar ha sido cerrado.');
            $('#body_modal_editar_monitor').html('');
        });
        ...
        $('#numero_sala').select2({
            ...
            dropdownParent: $("#form_editar_monitor")
        ...
        $('#btn_editar_form_monitor').on('click', function (event) {
            event.preventDefault();
           ...
           /*  const objeto_data = {
                monitor_id : document.getElementById('monitor_id_editar').value,
