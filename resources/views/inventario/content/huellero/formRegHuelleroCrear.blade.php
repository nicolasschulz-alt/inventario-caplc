<style>
    #modal_registrar_huellero .form-group select,
    #modal_registrar_huellero .form-group option,
    #modal_registrar_huellero .form-group input,
    #modal_registrar_huellero .form-group textarea,
    #modal_registrar_huellero .form-group label
    {
        /* font-size: 14px; */
        color: #3a3b45;
    }
...
<form action="{{asset('/inventario/huellero_create')}}" id="form_registrar_huellero" route="{{ route('inventario.huellero_create') }}" novalidate>
    @csrf
    <div class="row">
            ...
            <div class="form-group mb-1">
                ...
                <div class="input-group">
                    ...
                    <div class="input-group-append">
                        <button id="btn_comprobar_serie_huellero" class="btn btn-info btn-sm" type="button" id="button-addon2"><i class="fas fa-check-circle"></i></button>
                        <button id="btn_cambiar_serie_huellero" class="btn btn-warning btn-sm" type="button" id="button-addon2"><i class="fas fa-pen fa-sm"></i></button>
                    </div>
                    
                    <div class="serie-feedback invalid-feedback">
                        Ingrese número de serie (min. 6 caracteres)
                    </div>
                    <input type="hidden" id="route_comprobar_serie_huellero" value="{{ route('inventario.comprobar_serie_huellero') }}">
                </div>
            </div>
        </div>
        <div class="col-lg-4 col-md-12 col-sm-12">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 text-center">
            <button type="button" id="btn_guardar_form_huellero" route="{{ route('inventario.huellero_create') }}" class="btn btn-success btn-md">Guardar</button>
        </div>
    </div>
</form>

<script>
    var cerrar_modal_reg_huellero = false;
    $(document).ready(function () {

        //bloquea campos iniciales form registro PC
        bloqueaCamposFormRegistrarHuellero();

        $('#modal_registrar_huellero .close').on('click', function (event) {
           console.log(event);
           event.preventDefault(); // Previene el cierre del modal
           event.stopPropagation(); // Detiene la propagación del evento

           if (!cerrar_modal_reg_huellero){
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        cerrar_modal_reg_huellero = true;
                        $('#modal_registrar_huellero').modal('hide');
                        //$('#body_modal_registrar_huellero').html('');
                        console.log('confirma salir');
                        
                    } else {
                        cerrar_modal_reg_huellero = false;
                    }
                });
           }
        ...
        $('#modal_registrar_huellero').on('hidden.bs.modal', function () {
        ...
        $('#numero_sala').select2({
            ...
            dropdownParent: $("#form_registrar_huellero")
        ...
        $('#btn_cambiar_serie_huellero').on('click', function (event) {
            event.preventDefault();
            ...
            bloqueaCamposFormRegistrarHuellero();
            $('#btn_comprobar_serie_huellero').prop('disabled',false);
            setTimeout(() => {
                $('#serie').focus();
            }, 150);
        });

        $('#btn_comprobar_serie_huellero').on('click',async function (event) {
           event.preventDefault();
                ...
                if (val_serie && val_serie.length > 0 && val_serie.length < 6 ){
                    ...
                    await comprobar_serie_huellero(val_serie);
       ...
       /*  $('#ip_id').select2({
        ...
        $('#btn_guardar_form_huellero').on('click', function (event) {
            event.preventDefault();
            ...
            const objeto_data = {
                ...
                //huellero: document.getElementById('huellero').value,
    ...
    function bloqueaCamposFormRegistrarHuellero(){

        $('#form_registrar_huellero input').prop('disabled',true);
        $('#form_registrar_huellero select').prop('disabled',true);
        $('#form_registrar_huellero textarea').prop('disabled',true);
        $('#form_registrar_huellero button').prop('disabled',true);
        $('#form_registrar_huellero #serie').prop('disabled',false);
        $('#btn_comprobar_serie_huellero').prop('disabled',false);
        $('#btn_cambiar_serie_huellero').prop('disabled',false);
    ...
    function ActivaCamposFormRegistrarPC(){
        ...
        $('#form_registrar_huellero input').prop('disabled',false);
        $('#form_registrar_huellero select').prop('disabled',false);
        $('#form_registrar_huellero textarea').prop('disabled',false);
        $('#form_registrar_huellero button').prop('disabled',false);
    ...
    /* function bloqueaCamposAppsFormRegistrarPc(){
        ...
        $('#form_registrar_huellero #slc_apl_lic_so').prop('disabled',true);
        $('#form_registrar_huellero #slc_apl_lic_office').prop('disabled',true);
        $('#form_registrar_huellero #slc_apl_lic_antivirus').prop('disabled',true);
        $('#form_registrar_huellero select[id^=slc_apl_lic_app_]').prop('disabled',true);
        $('#form_registrar_huellero input[id^=licencia_app_]').prop('disabled',true);
    ...
    async function comprobar_serie_huellero(serie){

        console.log(serie);
        loader('show');
        try {
            
            const route = document.getElementById('route_comprobar_serie_huellero').value;
            ...
            if (data.status == 200){
                ...
                if (data.serie){
                    ...
                    ActivaCamposFormRegistrarPC();
                    //bloqueaCamposAppsFormRegistrarPc();
                    $('#btn_comprobar_serie_huellero').prop('disabled',true);
