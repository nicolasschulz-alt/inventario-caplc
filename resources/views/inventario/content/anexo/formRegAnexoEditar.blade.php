<style>
    #modal_editar_anexo .form-group select,
    #modal_editar_anexo .form-group option,
    #modal_editar_anexo .form-group input,
    #modal_editar_anexo .form-group textarea,
    #modal_editar_anexo .form-group label
    {
        /* font-size: 14px; */
        color: #3a3b45;
    }
...
<form action="{{asset('/inventario/anexo_editar')}}" id="form_editar_anexo" route="{{ route('inventario.anexo_update') }}" novalidate>
    @csrf
    <div class="row">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 text-center">
            <button type="button" id="btn_editar_form_anexo" route="{{ route('inventario.anexo_update') }}" class="btn btn-warning btn-md">Editar</button>
            <input type="hidden" id="anexo_id_editar" name="anexo_id_editar" value="{{$anexo_id}}">
        </div>
    </div>
</form>

<script>
    var cerrar_modal_edit_anexo = false;
    $(document).ready(function () {

        //bloquea campos iniciales form registro PC
        //bloqueaCamposFormRegistrarPC();

        $('#modal_editar_anexo .close').on('click', function (event) {
           console.log(event);
           event.preventDefault(); // Previene el cierre del modal
           event.stopPropagation(); // Detiene la propagación del evento

           if (!cerrar_modal_edit_anexo){
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        cerrar_modal_edit_anexo = true;
                        $('#modal_editar_anexo').modal('hide');
                        console.log('confirma salir');
                        
                    } else {
                        cerrar_modal_edit_anexo = false;
                    }
                });
           }
        ...
        $('#modal_editar_anexo').on('hidden.bs.modal', function (event) {
            console.log('El modal editar ha sido cerrado.');
            $('#body_modal_editar_anexo').html('');
        });
        ...
        $('#numero_sala').select2({
            ...
            dropdownParent: $("#form_editar_anexo")
        ...
        $('#btn_editar_form_anexo').on('click', function (event) {
            event.preventDefault();
           ...
           /*  const objeto_data = {
                anexo_id : document.getElementById('anexo_id_editar').value,
