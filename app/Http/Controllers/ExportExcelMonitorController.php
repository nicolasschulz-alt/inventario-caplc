...
class ExportExcelMonitorController extends Controller
{
    ...
    public function export()
    {
        ...
        //try {
            ...
            foreach ($monitor_list as $monitor) {
                $sheet->setCellValue('A' . $row, $monitor->monitor_id);
                $sheet->setCellValue('B' . $row, $monitor->serie);
                $sheet->setCellValue('C' . $row, $monitor->num_inventario);
                $sheet->setCellValue('D' . $row, $monitor->estado_dispositivo);
                $sheet->setCellValue('E' . $row, $monitor->nombre_propietario);
                $sheet->setCellValue('F' . $row, $monitor->nombre_edificio);
                $sheet->setCellValue('G' . $row, $monitor->nombre_piso);
    
                // Asignar el teléfono como texto (solo en la columna 'H')
                $sheet->setCellValueExplicit('H' . $row, $monitor->numero_sala, DataType::TYPE_STRING);

                $sheet->setCellValue('I'. $row, $monitor->nombre_sala);
                $sheet->setCellValue('J'. $row, $monitor->nombre_marca);
                $sheet->setCellValue('K'. $row, $monitor->nombre_modelo);
                $sheet->setCellValue('L'. $row, $monitor->observaciones);
                $sheet->setCellValue('M'. $row, $monitor->nombre_user_crea);
                $sheet->setCellValue('N'. $row, $monitor->fecha_user_crea);
                $sheet->setCellValue('O'. $row, $monitor->nombre_user_mod);
                $sheet->setCellValue('P'. $row, $monitor->fecha_user_mod);
